...
私有库
```angular2html
    仓库地址: http://gitlab.com/group/project

    go env -w GOPRIVATE=gitlab.com

    设置url替换 把https转换为ssh获取依赖
    git config --global url."git@gitlab.com".insteadOf "https://gitlab.com"
    如果遇到https报错,可以设置如下
    git config --global http.sslVerify false  # 禁用 SSL 验证（不推荐）

    如果拉取的是仓库的子目录则还需要配置
    生成 Token：GitLab -> Preferences → Access Tokens → 创建（read_repository + api）
    对于win,可以先设置一个Home环境变量,指向你的用户目录,然后在用户目录下创建_netrc文件;对于mac/linux则可以直接创建~/.netrc文件
    编辑~/.netrc文件
    machine gitlab.com //machine 指向gitlab仓库地址
    login dd //你的gitlab的用户名
    password dd //申请的token
```
